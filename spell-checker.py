...
def correct_sentence(s, lm, err_dist, c=2, alpha=0.95):
    ...
    sentence_word_candidates = [_generate_candidates_with_proba(w, error_dist) for w in sentence_words]
    sentence_candidates = []
    indexes_to_check = itertools.combinations(range(len(sentence_words)), c)
    # TODO add multiple words to change in the sentence according to c
    for i, word_to_check in enumerate(sentence_words):
        modified_words = sentence_word_candidates[i].keys()
...
if __name__ == '__main__':
    ...
    print evaluate_text(
        ...
        lm)
    print correct_sentence("how are yuo?", lm, error_dist)
    pass
